...
public class FlyoutContainer extends Composite {
            ...
            if (inside) {
                // check for state
                {
                    ...
                    if (flyoutControl.isEast()) {
                        stateHover = x > clientArea.width - titleHeight && y < titleHeight;
                    } else {
                        stateHover = x < titleHeight && y < titleHeight;
                    }
                ...
                if (isOpenExpanded()) {
                    if (flyoutControl.isWest() && x >= clientArea.width - RESIZE_WIDTH) {
                        ...
                        setCursor(ICursorConstants.SIZEWE);
                    } else if (flyoutControl.isNorth() && y >= clientArea.height - RESIZE_WIDTH) {
            ...
            Listener listener = new Listener() {
                public void handleEvent(Event event) {
                    if (event.type == SWT.Dispose) {
                        ...
                        if (flyoutControl.getClientArea().contains(flyoutControl.toControl(p.x, p.y))) {
                            getDisplay().removeFilter(SWT.MouseMove, this);
    ...
    private void handleDocking() {
        ...
        dockingTracker.addListener(SWT.Move, new Listener() {
            public void handleEvent(Event event2) {
                ...
                // check locations
                if (location.y < h3 && flyoutControl.isValidDockLocation(IFlyoutPreferences.DOCK_NORTH)) {
                ...
                } else if (location.y > 2 * h3 && flyoutControl.isValidDockLocation(IFlyoutPreferences.DOCK_SOUTH)) {
                    dockingTracker.setRectangles(new Rectangle[] { new Rectangle(0, clientArea.height - width,
                    ...
                    dockingTracker.setRectangles(new Rectangle[] { new Rectangle(clientArea.width - width, 0, width,
                            clientArea.height) });
    ...
    private void handlePaint(GC paintGC) {
        ...
        GC gc;
        {
            if (backImage == null || !backImage.getBounds().equals(clientArea)) {
                ...
                backImage = new Image(getDisplay(), clientArea.width, clientArea.height);
            }
            // prepare GC
            gc = new GC(backImage);
    ...
    private void drawStateImage(GC gc, int x, int y) {
        DrawUtils.drawImageCHCV(gc, getStateImage(), x, y, titleHeight, titleHeight);
        if (stateHover) {
            DrawUtils.drawHighlightRectangle(gc, x, y, titleHeight, titleHeight);
        }
    }
