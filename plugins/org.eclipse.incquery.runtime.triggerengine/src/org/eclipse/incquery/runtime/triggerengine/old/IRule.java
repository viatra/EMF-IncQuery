 ...
 * 
 * <p>
 * The interface allows acces to the precondition pattern, the corresponding agenda and the unmodifiable list of
 * activations.
 * 
 * <p>
 * The {@link #setStateChangeProcessor(ActivationState, IMatchProcessor)} method allows the registration of processors
 * that will be invoked when an activation changes states.
 * 
 * <p>
...
public interface IRule<MatchType extends IPatternMatch> extends IActivationNotificationProvider {
    ...
    /**
     * Registers a processor for the given state. The processor will be invoked, if an activation that is in the given
     * state is fired.
     * 
     * @param newState
     *            the state where the processor is used
     * @param processor
     *            the processor to use in the given state
     */
    void setStateChangeProcessor(ActivationState newState, IMatchProcessor<MatchType> processor);

    /**
     * Returns the processor registered for the given state. If no processor is registered, it returns null instead.
     * 
     * @param newState
     *            the state for which the processor is requested
