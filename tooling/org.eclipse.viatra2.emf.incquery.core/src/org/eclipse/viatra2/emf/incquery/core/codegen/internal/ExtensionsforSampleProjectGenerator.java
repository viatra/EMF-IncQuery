...
public class ExtensionsforSampleProjectGenerator {
	 ...
	 */
	public ExtensionsforSampleProjectGenerator(IProject project) throws CodeGenerationException {
		super();
		this.project = project;
	}
	
	private IPluginElement addIFileFilter(IExtensionsModelFactory factory, 
			IPluginExtension contribExtension,
		...
		IPluginElement builderElementWith = factory.createElement(contribExtension);
		builderElementWith.setName("with");
		builderElementWith.setAttribute("variable", "selection");
		builderElement.add(builderElementWith);
		
		//creates the iterate
		IPluginElement builderElementIterate = factory.createElement(contribExtension);
		...
		//creates the adapt
		IPluginElement builderElementAdapt = factory.createElement(contribExtension);
		builderElementAdapt.setName("adapt");
		builderElementAdapt.setAttribute("type", "org.eclipse.core.resources.IFile");
		builderElementIterate.add(builderElementAdapt);
		
		//creates the test
		IPluginElement builderElementTest = factory.createElement(contribExtension);
		builderElementTest.setName("test");
		builderElementTest.setAttribute("value", "*."+fileExtension);
		builderElementTest.setAttribute("property", "org.eclipse.core.resources.name");
		builderElementAdapt.add(builderElementTest);
		
		return builderElement;
	}
	
	private IPluginElement createMenuContribution(IExtensionsModelFactory factory, 
			IPluginExtension contribExtensionMenuContrib,
			String fileExtension,
			String menuID,
			Map<String, HandlerData> handlers) throws CoreException {
		
		IPluginElement builderElement = factory.createElement(contribExtensionMenuContrib);
		...
		IPluginElement builderElementMenu = factory.createElement(contribExtensionMenuContrib);
		...
		for (Map.Entry<String,SampleHandlerGenerator.HandlerData> entry : handlers.entrySet()) {
			IPluginElement builderElementCommand = factory.createElement(contribExtensionMenuContrib);
			builderElementCommand.setName("command");
			builderElementCommand.setAttribute("commandId", entry.getKey()+"ID");
			builderElementCommand.setAttribute("style", "push");
			builderElementCommand.add(this.addIFileFilter(factory, contribExtensionMenuContrib, fileExtension));
	...
	public void contributeToExtensionPoint(Map<String, HandlerData> handlers,
		...
		try {
			...
			IPluginExtension contribExtensionMenuContrib = factory.createExtension();
			contribExtensionMenuContrib.setId(extensionID);
			contribExtensionMenuContrib.setPoint("org.eclipse.ui.menus");
			
			IPluginElement menuElement = this.createMenuContribution(factory, contribExtensionMenuContrib, fileExtension,
					"popup:org.eclipse.ui.navigator.ProjectExplorer#PopupMenu",
					handlers);
			contribExtensionMenuContrib.add(menuElement );
			
			menuElement = this.createMenuContribution(factory, contribExtensionMenuContrib, fileExtension,
					"popup:org.eclipse.ui.views.ResourceNavigator",
					handlers);
			contribExtensionMenuContrib.add(menuElement );
			...
			IPluginExtension contribExtensionHandler = factory.createExtension();
			contribExtensionHandler.setId(extensionID);
			contribExtensionHandler.setPoint("org.eclipse.ui.handlers");
			for (Map.Entry<String,SampleHandlerGenerator.HandlerData> entry : handlers.entrySet()) {
				IPluginElement builderElementHandler = factory.createElement(contribExtensionHandler);
				builderElementHandler.setName("handler");
				builderElementHandler.setAttribute("class", entry.getValue().getHandlerPackage()+"."+entry.getKey());
				builderElementHandler.setAttribute("commandId", entry.getKey()+"ID");
			...
			throw new CodeGenerationException("Error during EMF-IncQuery Sample project generation. ", e1);
		}
	}
